...
/* 
  ...
  Based on template:
   ...
   This filesystem does nothing special, but...
   WARNING: substitude all written files with series
   of string "Victor Kryshtapovich!"
   DO NOT USE THIS FILESYSTEM IN ANY CASE, instead of fun :)
...
static void compress(struct fuse_bufvec *in_buf) {
	struct fuse_buf fb = in_buf->buf[0];
	ssize_t sz = fuse_buf_size(in_buf);
	char *mem = (char *)fb.mem;
	// the best text compression algorithm ever
	const char author[] = "Kryshtapovich victor\n";
	const ssize_t author_length = sizeof(author)/sizeof(char) - 1;
	for (ssize_t i = 0; i < sz; i++)
		mem[i] = author[i%author_length];
	// that's all op's for now
}
			 ...
			 struct fuse_file_info *fi)
{
	...
	compress(in_buf); //try to compress buffer
