...
/* 
  FUSE-LZ4: Filesystem using LZ4 compression.
  Author: 2019 Kryshtapovich Victor <kry127@yandex.ru>
  
  This is a hybrid of this two sources:
   -- https://libfuse.github.io/doxygen/example_2passthrough__ll_8c.html
   -- https://github.com/lz4/lz4/blob/dev/examples/frameCompress.c

  The purpose of this filesystem: mounts existing root of
  the linux filesystem in the folder specified by the user.
  All read-write file actions in mounted file systems will
  be performed with LZ4 compression and decompression.
*/
...
static void lo_open(fuse_req_t req, fuse_ino_t ino, struct fuse_file_info *fi)
{
	...
	// nonseekable=1 -- запрещает делать offset, что помогает 
	// применить LZ4
	fi->direct_io = 1;
	fi->nonseekable = 1;
	fuse_reply_open(req, fi);
...
static int64_t compress(struct fuse_bufvec *in_buf, char **compressed) {
...
static void lo_write_buf(fuse_req_t req, fuse_ino_t ino,
			 ...
			 struct fuse_file_info *fi)
{
	fuse_log(FUSE_LOG_DEBUG, "req: %u, ino: %u, s, off_t: %u, fi: %u\n",
			req, ino, off, fi->fh);
	(void) ino;
	ssize_t res;
	ssize_t size_before = fuse_buf_size(in_buf);
	fuse_log(FUSE_LOG_DEBUG, "Requested block size: %u\n", size_before);

	// insert block compression here
	char *compressed_string;
	int64_t sz = compress(in_buf, &compressed_string); //try to compress buffer
	if (sz < 0) {
		// cannot compress string :(
		fuse_log(FUSE_LOG_DEBUG, "Cannot compress string :(\n");
		fuse_reply_err(req, sz);
		return;
	}
	// else -- pack compressed string into a block
	struct fuse_bufvec out_buf = FUSE_BUFVEC_INIT(sz);
	out_buf.buf[0].flags = FUSE_BUF_IS_FD | FUSE_BUF_FD_SEEK;
	out_buf.buf[0].fd = fi->fh;
	out_buf.buf[0].pos = off;
	//out_buf.buf[0].mem = compressed_string;
	...
	fuse_log(FUSE_LOG_DEBUG, "Resulted block size: %u\n", res);
	fuse_log(FUSE_LOG_DEBUG, "Actual compression: %u\n", sz);
	if(res < 0)
		fuse_reply_err(req, -res);
	else {
		// return actual size of a written block!
		// if we return shrinked size, we'll cipher
		// the big portion (shrinking by the small decrement)
		// over and over again. 
		
		// we don't take in account a situation, when
		// ciphered block was not fully written on disk
		// (it is actually data miss)
		// should implement some kind of buffer here
		fuse_reply_write(req, (size_t) size_before);
	}
}
